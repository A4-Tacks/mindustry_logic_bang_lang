...
> [!IMPORTANT]
> 以上的逻辑变量并不完全在 Bang 适用, 比如 `a-b` 和 `let's`,
...
但是这样随机生成的 Var 我们就不知道它叫什么了, 所以**使用返回句柄`$`来引用它**
...
初探常用控制流
===============================================================================
在 Bang 中, 有许多方便的控制语句可以使用, 如`if` `while`等
...
可以分别使用`break`  `continue`语句, 直接跳出循环或跳到新一轮循环
...
__3:
    ...
    print 2
```
第一种选择了跳转表形式生成, 第二种选择了对齐块形式生成,
并且没手动加上跳出的话, 会跳转到第i块, 然后以第i块开始将大于i的所有块都执行一遍

通常我们并不需要这样, 而是执行某一块结束后, 直接跳出整个部分,
所以可以使用`select`的进阶, `switch`

`switch`使用`case`来分隔每一块代码,
同时还可以将每个case前的代码附加到每块`case`的最后, 称为 switch-append
...
简化运算 - 运算表达式 (op-expr)
-------------------------------------------------------------------------------
这让人们可以以传统的优先级、方便的形式编写原始的DExp形式
...
print 2
```
使用方案A会在编译结果中得到`__0`和`__1`这两个生成的句柄,
