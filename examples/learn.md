...
> [!IMPORTANT]
> 该语言建议使用 VSCode 或 VIM (coc.nvim) 进行编写,
> - 对于VIM提供了 [VIM插件](../syntax/vim) 包含 ultisnips 定义
> - 对于VSCode提供了插件, 在 [Releases] 中文件后缀为 `.vsix` 的插件, 在 VSCode 中导入
>
> 同时提供了[语言服务器](../tools/bangls), 在 [Releases] 中前缀为 `bangls` 的程序,
> 在进阶学习时建议使用
>
> 提供了[反编译器](../tools/decompiler), 用于将逻辑代码转换为 bang 控制流
