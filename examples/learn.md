...
---
ResultHandle (返回句柄[^4]), 这也是一种值, 在 DExp 中使用,
...
---
ReprVar 原始量, 在之后的常量系统里面会讲, 写法是普通的量包裹一层反引号
...
---
ValueBind 值绑定, 可以将一个量绑定到一个值的求值结果上, 也就是这两个量,
...
- Closure: 正常的展开内部值, 但是会先设置闭包捕获的环境
...
## if elif else skip while do-while gwhile break continue
这些也是语法糖, 之前介绍过了
